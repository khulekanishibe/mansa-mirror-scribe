...
const QuestionArea = ({ currentQuestion, currentSubQuestion = 1 }: QuestionAreaProps) => {
              ...
              <table className="w-full border-collapse border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
                <thead>
                  <tr className="bg-[#0d643f] text-white">
                    {info.table.columns.map((col: string, index: number) => (
                      <th key={index} className="border border-gray-300 dark:border-gray-600 p-3 text-left">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {info.table.rows.map((row: string[], rowIndex: number) => (
                    <tr key={rowIndex} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                      {row.map((cell: string, cellIndex: number) => (
                        <td key={cellIndex} className="border border-gray-300 dark:border-gray-600 p-3 text-gray-700 dark:text-gray-300">
                          {cell}
                        </td>
                      ))}
                    </tr>
          ...
          {info.labourCostPerUnit && (
            <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
              ...
              <ul className="space-y-1 text-sm text-orange-800 dark:text-orange-300">
                {info.labourCostPerUnit.map((item: string, index: number) => (
